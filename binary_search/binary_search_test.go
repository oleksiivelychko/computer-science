...
func TestBinarySearch(t *testing.T) {
	...
	if !result && index != 2 {
		t.Errorf("[func BinarySearch(slice []int, target int) (bool, int)] -> (%t, %d) != (true, 2)", result, index)
	}
}
...
func TestBinarySearchRecursion(t *testing.T) {
	...
	if !result && index != 4 {
		t.Errorf("[func BinarySearchRecursion(slice []int, target int) (bool, int)] -> (%t, %d) != (true, 4)", result, index)
	}
}
